...
function getWeeklyCashflow({ pastWeeks = 1, futureWeeks = 2, scenarioId = null, anchor = null } = {}) {
  ...
  const evolutiveStartDate = scenarioRow ? scenarioRow.evolutive_funnel_start_date : null;
  const overridesMap = scenarioId ? weeklyOverrides.mapByScenario(scenarioId) : new Map();

  function evolutiveIndexForWeek(week) {
    if (!evolutiveStartDate) return null;
    try {
      const startSun = weekRangeFromDate(evolutiveStartDate).sun;
      const wkSun = weekRangeFromDate(week.sun).sun;
      const diffDays = Math.round((wkSun - startSun) / (24 * 60 * 60 * 1000));
  ...
  for (const w of weeks) {
    ...
    if (w.is_future) {
      if (overridesMap.has(w.week_id)) {
        ...
        let idx;
        if (evolutiveStartDate) {
          idx = evolutiveIndexForWeek(w);
        } else {
          evoWeekIndexAuto += 1;
          idx = evoWeekIndexAuto;
        }
        salesProj = idx ? getProjectedNetForWeek(scenarioId, idx) : 0;
      } else {
  ...
  for (const w of weeks) {
    ...
    if (w.is_past) {
      ...
      const salesValue = data.sales_projected_overridden
        ? Math.max(data.sales_projected, data.sales_real)
        : data.sales_real;
      delta = salesValue - data.costs_paid - data.costs_planned - adsContrib;
    } else {
